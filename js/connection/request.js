    ...
    return {
        ...
        init: (callback, lists = []) => {
            if (!window.isSecureContext) {
                console.warn('Application is not in a secure context. Native caching is disabled.');
            }
            ...
            if (!window.caches) {
                ...
                lists.concat([cacheRequest]).forEach((v) => cachePool.set(v, mockCache));
                return callback();
            }
    ...
    */
    ...
    return {
        ...
        send(transform = null) {
            ...
            let absoluteBase = baseUrl === './' ? window.location.href.split('?')[0].replace(/[^/]+$/, '') : baseUrl;
            let targetUrl = path.startsWith('/api/') ? absoluteBase + 'api.php?route=' + encodeURIComponent(path) : new URL(path, absoluteBase).href;
